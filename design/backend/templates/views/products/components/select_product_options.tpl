...
<div class="cm-switch-availability-container" id="sw_option_{$id}_AOC_suffix">
<input type="hidden" name="appearance[id]" value="{$id}" />
<input type="hidden" name="appearance[name]" value="{$name}" />

{if $only_selectable_options}
    <input type="hidden" name="appearance[only_selectable_options]" value="{$only_selectable_options}" />
    <input type="hidden" name="additional_info[get_only_selectable_options]" value="{$only_selectable_options}" />
{/if}
...
<div class="controls">
    {if $po.option_type == "ProductOptionTypes::SELECTBOX"|enum} {*Selectbox*}
        ...
        {else}
            {__("na")}
            <input type="hidden" name="{$name}[{$id}][product_options][{$po.option_id}]" value="{$po.value}" id="option_{$id}_{$po.option_id}" {if $cp.exclude_from_calculate && !$product.aoc || $po.disabled}disabled="disabled"{/if}/>
        {/if}
    ...
    {elseif $po.option_type == "ProductOptionTypes::CHECKBOX"|enum} {*Checkbox*}
        ...
        {foreach from=$po.variants item="vr"}
        {if $vr.position == 0}
            <input id="unchecked_{$id}_option_{$po.option_id}" {if $cp.exclude_from_calculate && !$product.aoc || $po.disabled}disabled="disabled"{/if} type="hidden" name="{$name}[{$id}][product_options][{$po.option_id}]" value="{$vr.variant_id}" />
        {else}
    ...
    {elseif $po.option_type == "ProductOptionTypes::INPUT"|enum} {*Input*}
        <input id="option_{$id}_{$po.option_id}" type="text" name="{$name}[{$id}][product_options][{$po.option_id}]" value="{$po.value|default:$po.inner_hint}" {if $cp.exclude_from_calculate && !$product.aoc}disabled="disabled"{/if} class="input-text {if $po.inner_hint}cm-hint{/if}" {if $po.inner_hint}title="{$po.inner_hint}"{/if} {if $update_options}onchange="fn_change_options('{$c_obj|default:$id}', {$id}, '{$po.option_id}');"{/if} />
    {elseif $po.option_type == "ProductOptionTypes::TEXT"|enum} {*Textarea*}
        <textarea id="option_{$id}_{$po.option_id}" name="{$name}[{$id}][product_options][{$po.option_id}]" {if $cp.exclude_from_calculate}disabled="disabled"{/if} class="input-textarea-long {if $po.inner_hint && $po.value == ""}cm-hint{/if}" {if $po.inner_hint}title="{$po.inner_hint}"{/if} {if $update_options}onchange="fn_change_options('{$c_obj|default:$id}', {$id}, '{$po.option_id}');"{/if}>{$po.value|default:$po.inner_hint}</textarea>
